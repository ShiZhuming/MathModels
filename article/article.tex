...
\begin{document}
    ...
    \begin{equation}
        ...
        \end{cases}
    \end{equation}\par
    在问题1中$Q_{in}(t)$的形式也比较简单，我们定义$0-1$变量$\lambda=\lambda(t)$来描述供油处入口的截面积，它的形式为
    \begin{equation}
        \lambda(t)=
        \begin{cases}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[scale=1]{figure/data3.png}
    ...
    % bug!!!
    首先我们定义等效面积函$B(t)$，用来描述针阀运动过程中喷油嘴喷孔的等效面积。由简单的几何计算可知，它的形式如下。
    \begin{equation}
        B(t)=\pi\left(\left(\frac{d_B}{2}+h(t)\tan9^\circ\right)^2-\left(\frac{d_B}{2}\right)^2\right)
    \end{equation}
    其中的$h(t)$为针阀升程，由题目附件2给出的数据拟合得到，其图形和拟合结果如下
    \begin{equation}
        h(t)=
        \begin{cases}
            \frac{0.5342(t-100k)^2-0.04835(t-100k)+0.000726}{(t-100k)^2-0.7362(t-100k)+0.1716}&100k\leq t\leq 100k+0.45,k\in\mathbb{N}\\
            2&100k+0.45\leq t\leq 100k+2,k\in\mathbb{N}\\
            \frac{0.5358(t-100k)^2-2.576(t-100k)+3.096}{(t-100k)^2-4.163(t-100k)+4.368}&100k+2\leq t\leq 100k+2.45,k\in\mathbb{N}\\
    ...
    \end{equation}\par
    再来看$Q_{in}$的表达形式，它比较复杂，所以我们不解出其具体形式，而将其用一个隐函数描述。考虑油泵油缸的质量守恒，可以得到下面的方程（以下下标为$h$的项均为油泵油缸的性质）。
    \begin{equation}
    ...
    \begin{equation}
        ...
        \label{higheq2}
    \end{equation}
    将方程\ref{pandrho}、\ref{highmaineq}、\ref{higheq2}联立可以，整理得到
    \begin{equation}
        \frac{\dif p_h}{E(p_h)\dif t}=\frac{-Q_{in}\frac{\rho}{\rho_h}-\frac{\dif V_h}{\dif t}}{V_h}
        \label{highmaineq2}
    \end{equation}\par
    在高压油泵供油的过程中我们可以用方程\ref{highmaineq2}去描述它的情况。\par
    这个方程的形式和刚才对高压油管的建模模型——方程\ref{maineq}形式很相似，也就是我们可以把高压油泵的油缸视作另一个高压油管，活塞对它的压缩可以视作$Q_{in}$，而向高压油管供油可以视作$Q_{out}$，当然此时$V$是变量，所以不可以直接代入。
